...
namespace gz
{
    ...
    ///
    /// \param path: the full file path (directory/name.extension)
    ///
    ...
    ////////////////////////////////////////////////////////////
    /// \brief returns that directory of the file
    ///
    /// \param path: the full file path (directory/name.extension)
    ///
    /// \return the extension from the file directory
    ...
    inline std::string getFileName(std::string path)
    {
        ...
        if (last_index == 0)        return ""; // there is no filename
        else if (last_index > 0)    last_index -= 1;
    ...
    {
        int last_index = gz::lastIndexOf(path,'/');

        if (last_index <= 0)    return "";
        else                    return gz::subString(path,0,last_index);
    }
